...
<#macro inputTag attribute value>
	<input type="<@inputTagType attribute=attribute/>"
		   ...
		   id="${attribute.name}" name="${attribute.name}" value="${(value!'')}" class="${properties.kcInputClass} <#if messagesPerField.existsError('${attribute.name}')>${properties.kcInvalidInputClass}</#if>""
